...
contract MelioraAuction is Pausable,HasNoEther {
 ...
 constructor(uint256 _ownerCut) {
    ...
    whenNotPaused
  {
    ...
    _bid(_nftAddress,_payTokenAddress, _tokenId, payAmount);
    ...
    returns (uint256)
  {
    ...
    if (_price > 0) {
      ...
      IERC20(_payTokenAddress).safeTransferFrom(msg.sender,address(this),_auctioneerCut);
      uint256 _sellerProceeds = _price - _auctioneerCut;
      IERC20(_payTokenAddress).safeTransferFrom(msg.sender,_seller,_sellerProceeds);
    }
